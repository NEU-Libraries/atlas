...
#
# Walking to the parent costs a query per hop, and the walk stops at the first
# Work or container. That is deliberate: this runs on the binary read path,
# where correctness outranks the round trip, and never on the Work read path.
module LeafReadAuthority
  extend ActiveSupport::Concern

  # Leaves sit at varying depth: a Blob two below its Work, a page's Service
  # File Delegate three (Delegate > derivative FileSet > page FileSet > Work).
  # This cap is not a depth limit. It only stops a corrupt parent chain from
  # looping — Relationships#ancestor_resources raises on a cycle, but this walk
  # is its own and must not depend on that.
  MAX_HOPS = 8
