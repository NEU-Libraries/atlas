...
# The bare Compilation keys, rendered both wrapped (show) and flat (index
# rows), so Cerberus chip counts read the same keys either way. Takes a
# `compilation` local rather than an ivar so index can pass each item.
